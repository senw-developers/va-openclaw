...
describe("runCliAgent cron before_agent_reply seam", () => {
  ...
  it("invokes before_agent_reply for user triggers (gateway ingress)", async () => {
    hasHooksMock.mockImplementation((hookName) => hookName === "before_agent_reply");
    executePreparedCliRunMock.mockResolvedValue({ text: "real reply" });
  ...
  it("lets a hook claim a user trigger before the CLI subprocess is invoked", async () => {
    hasHooksMock.mockImplementation((hookName) => hookName === "before_agent_reply");
    runBeforeAgentReplyMock.mockResolvedValue({ handled: true, reply: { text: "denied" } });
