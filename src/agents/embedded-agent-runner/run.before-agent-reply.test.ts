// Coverage for the before_agent_reply admission seam ahead of embedded attempts.
...
describe("runEmbeddedAgent before_agent_reply admission seam", () => {
  beforeAll(async () => {
  ...
  it("invokes before_agent_reply for user-triggered runs (gateway ingress)", async () => {
    // Gateway ingress (HTTP /v1/responses, WS chat.send, node events) reaches
    // this runner with trigger "user" hardcoded; a cron-only gate here left
    // those turns unclaimable by admission plugins.
    mockedGlobalHookRunner.hasHooks.mockImplementation(
      (hookName: string) => hookName === "before_agent_reply",
    );
    mockedRunEmbeddedAttempt.mockResolvedValueOnce(makeAttemptResult({ promptError: null }));
  ...
  it("lets a hook claim a user-triggered run before the attempt starts", async () => {
    mockedGlobalHookRunner.hasHooks.mockImplementation(
      (hookName: string) => hookName === "before_agent_reply",
    );
    mockedGlobalHookRunner.runBeforeAgentReply.mockResolvedValueOnce({
      handled: true,
      reply: { text: "credits exhausted" },
    });
    ...
    expect(mockedRunEmbeddedAttempt).not.toHaveBeenCalled();
    expect(result.meta.finalAssistantVisibleText).toBe("credits exhausted");
  });
