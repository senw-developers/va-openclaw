 ...
 *
 * Lifecycle:
 *  - gateway_start             → log readiness, probe backend health.
 *  - periodic (default 6h)     → re-probe; tokens are user-driven and
 *                                not pre-warmed.
 *  - nabu.googleworkspace.refresh → RPC NestJS calls to invalidate the
 *                                in-memory access-token cache (after
 *                                rotate / re-consent / disconnect).
 *  - gateway_stop              → clear the cache and timer.
 *
