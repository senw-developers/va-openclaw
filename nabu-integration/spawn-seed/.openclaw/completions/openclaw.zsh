...
_openclaw_plugins() {
  ...
  _arguments -C \
     \
    "1: :_values 'command' 'list[List discovered plugins]' 'info[Show plugin details]' 'enable[Enable a plugin in config]' 'disable[Disable a plugin in config]' 'uninstall[Uninstall a plugin]' 'install[Install a plugin (path, archive, npm spec, or marketplace entry)]' 'update[Update installed plugins (npm and marketplace installs)]' 'doctor[Report plugin load issues]' 'marketplace[Inspect Claude-compatible plugin marketplaces]'" \
    "*::arg:->args"
